...
/**
 * Example rescorer that multiplies the score of the hit by some factor and
 * doesn't resort them.
 */
public class ExampleRescoreBuilder extends RescorerBuilder<ExampleRescoreBuilder> {
	...
	private static final ConstructingObjectParser<ExampleRescoreBuilder, Void> PARSER = new ConstructingObjectParser<>(
			NAME, args -> new ExampleRescoreBuilder((float) args[0], (String) args[1]));
	static {
	...
	public RescoreContext innerBuildContext(int windowSize, SearchExecutionContext context) throws IOException {
		IndexFieldData<?> factorField = this.factorField == null ? null
	...
	private static class ExampleRescorer implements Rescorer {
		...
		public TopDocs rescore(TopDocs topDocs, IndexSearcher searcher, RescoreContext rescoreContext)
			...
			if (context.factorField != null) {
				/*
				 * Since this example looks up a single field value it should access them in
				 * docId order because that is the order in which they are stored on disk and we
				 * want reads to be forwards and close together if possible.
				 *
				 * If accessing multiple fields we'd be better off accessing them in (reader,
				 * field, docId) order because that is the order they are on disk.
				 */
					...
					if (topDocs.scoreDocs[i].doc >= endDoc) {
						...
						if (false == (fd instanceof LeafNumericFieldData)) {
							throw new IllegalArgumentException(
									"[" + context.factorField.getFieldName() + "] is not a number");
						}
